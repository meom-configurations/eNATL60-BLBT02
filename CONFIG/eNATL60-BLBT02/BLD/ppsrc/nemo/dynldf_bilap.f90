...
! NB: ahm is negative when bilaplacian is used
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1
                  z2d(ji,jj) = z2d(ji,jj)                                                                  &
                     &  +  (  ahm3(ji,jj)*zlu(ji,jj,jk)**2 + ahm3(ji-1,jj)*zlu(ji-1,jj,jk)**2    &
                     &      + ahm4(ji,jj)*zlv(ji,jj,jk)**2 + ahm4(ji,jj-1)*zlv(ji,jj-1,jk)**2  ) * tmask(ji,jj,jk)
               END DO
            END DO
         END DO
         zzz = 0.5_wp * rau0
