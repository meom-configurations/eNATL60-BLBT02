...
!LOLO:
      IF( iom_use("e3u") ) CALL iom_put( "e3u" , e3u_n(:,:,:) )
      IF( iom_use("e3v") ) CALL iom_put( "e3v" , e3v_n(:,:,:) )
      IF( iom_use("e3w") ) CALL iom_put( "e3w" , e3w_n(:,:,:) )
!LOLO.
      IF( iom_use("e3tdef") )   &
...
!
      IF ( iom_use("eken") ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zztmp   =  1 / (e1e2t(ji,jj) * e3t_n(ji,jj,jk))
                  zztmpx  = 0.5 * (  un(ji-1,jj,jk) * un(ji-1,jj,jk) * e1u(ji-1,jj) * e2u(ji-1,jj) * e3u_n(ji-1,jj,jk)    &
                     &             + un(ji  ,jj,jk) * un(ji  ,jj,jk) * e1u(ji,  jj) * e2u(ji  ,jj) * e3u_n(ji,jj,jk) )  &
                     &          *  zztmp
!
                  zztmpy  = 0.5 * (  vn(ji,jj-1,jk) * vn(ji,jj-1,jk) * e1v(ji,jj-1) * e2v(ji,jj-1) * e3v_n(ji,jj-1,jk)    &
                     &             + vn(ji,jj  ,jk) * vn(ji,jj  ,jk) * e1v(ji,jj  ) * e2v(ji,jj  ) * e3v_n(ji,jj,jk) )  &
                     &          *  zztmp
!
