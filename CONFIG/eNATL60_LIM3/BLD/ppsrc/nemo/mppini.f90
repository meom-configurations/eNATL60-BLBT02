...
!!
      ...
      INTEGER(1), DIMENSION(:,:), ALLOCATABLE :: imask  !LOLO  ! temporary global workspace
      REAL(wp),   DIMENSION(:,:), ALLOCATABLE :: zdta   !LOLO
...
!LOLO:
      ALLOCATE ( imask(jpiglo,jpjglo) , zdta(jpiglo,jpjglo) ) 
      imask(:,:) = 0
      zdta(:,:)  = 0.0_wp
!LOLO.
...
!CALL iom_getatt(inum, 'open_ocean_jstart', jstartrow ) ! -999 is returned if the attribute is not found
!jstartrow = MAX(1,jstartrow)
!WRITE(numout,*) ' ***LOLO: About to read Bathymetry in bathy_meter.nc, jpizoom, jpjzoom,jpiglo,jpjglo =', jpizoom, jpjzoom, jpiglo,jpjglo
